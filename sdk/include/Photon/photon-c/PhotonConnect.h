...
#if defined _EG_APPLE_PLATFORM || defined _EG_ANDROID_PLATFORM // Apple and Google already define them theirselves, but in a really weird way (on Apple platforms y=HTONL(x) changes byteorder in dest and source(!), not only in dest like htonl(x) and like everyone would expect)
#	undef HTONS
#	undef HTONL
#endif
#define HTONS htons
