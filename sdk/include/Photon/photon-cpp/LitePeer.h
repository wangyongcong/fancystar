...
namespace ExitGames
{
	...
	public:
		...
		virtual bool opRaiseEvent(bool sendReliable, const Hashtable& evData, nByte eventCode, nByte channelID=0, int* targetActors=NULL, short numTargetActors=0);
		virtual bool opRaiseEvent(bool sendReliable, const Hashtable& evData, nByte eventCode, nByte channelID, nByte eventCaching, nByte receiverGroup=RG_OTHERS);
		virtual bool opJoin(const JString& gameId, const Hashtable& gameProperties=Hashtable(), const Hashtable& actorProperties=Hashtable(), bool broadcastActorProperties=false);
		virtual bool opLeave(const JString& gameId);
		virtual bool opSetPropertiesOfActor(int actorNr, const Hashtable& properties, bool broadcast, nByte channelID=0);
		virtual bool opSetPropertiesOfGame(const Hashtable& properties, bool broadcast, nByte channelID=0);
		virtual bool opGetProperties(nByte channelID=0);
		virtual bool opGetPropertiesOfActor(const JString* const properties, short numProperties, const int* const actorNrList=NULL, short numActors=0, nByte channelID=0);
		...
		virtual bool opGetPropertiesOfGame(const JString* const properties, short numProperties, nByte channelID=0);
