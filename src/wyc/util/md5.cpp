...
void md5_init (MD5_CTX *mdContext, unsigned long pseudoRandomNumber)
{
...
void md5_hexdigest(MD5_CTX *mdContext, char out[32])
{
	const char *hex = "0123456789abcdef";
	unsigned char *c = mdContext->digest;
	for(int i=1; i<32; i+=2, ++c) {
		out[i-1] = hex[*c>>4];
		out[i] = hex[*c&0xF];
	}
}
